...
(define-prim (##udp-socket-tos port)

  (macro-port-mutex-lock! port) ;; get exclusive access to port

  (let ((result
         (##os-device-udp-socket-tos
          (macro-condvar-name
           (or (macro-udp-port-rdevice-condvar port)
               (macro-udp-port-wdevice-condvar port))))))
...
(define-prim (##udp-socket-tos-set! port size)

  (macro-port-mutex-lock! port) ;; get exclusive access to port

  (let ((result
         (##os-device-udp-socket-tos-set!
          (macro-condvar-name
           (or (macro-udp-port-rdevice-condvar port)
               (macro-udp-port-wdevice-condvar port)))
          size)))
...
(define-prim (##udp-socket-send-again-count port)

  (macro-port-mutex-lock! port) ;; get exclusive access to port

  (let ((result
         (##os-device-udp-socket-send-again-count
          (macro-condvar-name
           (or (macro-udp-port-rdevice-condvar port)
               (macro-udp-port-wdevice-condvar port))))))
                   ...
                   (let ((str (make-string i c2)))
                     ...
                     (let* ((suff-l (eat-either re #\l #\L))
                            ...
                            (suff-l2 (and (not suff-l)
                                          suff-u
                                          (eat-either re #\l #\L)))
                            ;; add support for D suffix
                            (suff-f/d (and (> state 0)
                                           (not suff-l)
                                           (not suff-u)
                                           (or (eat-either re #\f #\F)
                                               (eat-either re #\d #\D)))))
                       ;; quick solution
                       (if (and (= state 0)
                                (not suff-l)
